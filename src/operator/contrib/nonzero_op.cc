...
*/
#include "nonzero_op-inl.h"

namespace mxnet {
namespace op {

bool ContribNonzeroType(const nnvm::NodeAttrs& attrs,
...
bool ContribNonzeroStorageType(const nnvm::NodeAttrs& attrs,
                        const int dev_mask,
...
void ContribNonzeroForwardCPU(const nnvm::NodeAttrs& attrs,
                       const OpContext &ctx,
  ...
  MXNET_NDIM_SWITCH(in.shape().ndim(), ndim, {
    ...
    mxnet_op::Kernel<ContribNonzeroForwardKernel, cpu>::Launch(
      stream, in_size, out.data().dptr<int64_t>(), prefix_sum.data(), shape);
  })
}

NNVM_REGISTER_OP(_contrib_nonzero)
.set_num_inputs(1)
.set_num_outputs(1)
...
.set_attr<nnvm::FInferType>("FInferType", ContribNonzeroType)
.set_attr<FComputeEx>("FComputeEx<cpu>", ContribNonzeroForwardCPU)
.set_attr<FInferStorageType>("FInferStorageType", ContribNonzeroStorageType)
