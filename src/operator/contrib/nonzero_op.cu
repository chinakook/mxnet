...
void ContribNonzeroForwardGPU(const nnvm::NodeAttrs& attrs,
                       const OpContext &ctx,
  ...
  MXNET_NDIM_SWITCH(in.shape().ndim(), ndim, {
    ...
    mxnet_op::Kernel<ContribNonzeroForwardKernel, gpu>::Launch(
...
.set_attr<FComputeEx>("FComputeEx<gpu>", ContribNonzeroForwardGPU);
