...
struct ContribTriuParam : public dmlc::Parameter<ContribTriuParam> {
  int k;
  DMLC_DECLARE_PARAMETER(ContribTriuParam) {
...
inline bool ContribTriuOpShape(const nnvm::NodeAttrs& attrs,
...
void ContribTriuOpProcess(const TBlob& in_data,
                   const TBlob& out_data,
                   index_t dsize,
                   const ContribTriuParam& param,
  ...
  if (ishape.ndim() == 2 && oshape.ndim() == 2) {
    MSHADOW_TYPE_SWITCH(out_data.type_flag_, DType, {
      MXNET_ASSIGN_REQ_SWITCH(req[0], req_type, {
        Kernel<Contribtriu2D<req_type>, xpu>::Launch(
    ...
    MSHADOW_TYPE_SWITCH(out_data.type_flag_, DType, {
      MXNET_ASSIGN_REQ_SWITCH(req[0], req_type, {
        Kernel<Contribtriu3D<req_type>, xpu>::Launch(
            s, dsize, out_data.dptr<DType>(), in_data.dptr<DType>(),
    ...
    MSHADOW_TYPE_SWITCH(out_data.type_flag_, DType, {
      MXNET_ASSIGN_REQ_SWITCH(req[0], req_type, {
        if (back) {
          Kernel<Contribtriu1Dbackward<req_type>, xpu>::Launch(
              s, dsize, out_data.dptr<DType>(), in_data.dptr<DType>(),
              Shape1(oshape[0]), param.k);
        } else {
          Kernel<Contribtriu1Dforward<req_type>, xpu>::Launch(
              s, dsize, out_data.dptr<DType>(), in_data.dptr<DType>(),
              Shape2(oshape[0], oshape[1]), param.k);
        }
      });
...
void ContribTriuOpForward(const nnvm::NodeAttrs& attrs,
                   const OpContext& ctx,
  ...
  const ContribTriuParam& param = nnvm::get<ContribTriuParam>(attrs.parsed);

  ContribTriuOpProcess<xpu, false>(in_data, out_data, out_data.Size(), param, s, req);
}

template<typename xpu>
void ContribTriuOpBackward(const nnvm::NodeAttrs& attrs,
                    const OpContext& ctx,
                    const std::vector<TBlob>& inputs,
                    const std::vector<OpReqType>& req,
                    const std::vector<TBlob>& outputs) {
  ...
  ContribTriuOpProcess<xpu, true>(in_data, out_data, out_data.Size(), param, s, req);
}
